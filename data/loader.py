...
# for loading the dataset in data
class spatio_temporal_dataset(Dataset):
    ...
    def __getitem__(self, idx):
        if self.mode == 'train' or 'val':
            ...
            self.clips_idx = 1
            # taking 10 spatial frames that are 5 framses apart
            clips = [5,10,15,20,25,30,35,40,45,49]
        else:
...
class spatio_temporal_dataloader():
    def __init__(self, BATCH_SIZE, num_workers, in_channel, spatial_path, temp_path, ucf_list, ucf_split,
        ...
        splitter = spilt.UCF101_splitter(path=ucf_list, split=ucf_split)
        self.train_video, self.test_video = splitter.split_video()

    def load_frame_count(self):
        # loading the frame count for videos
        with open('/home/chunyan/two_stream/data/dic/frame_count.pickle', 'rb') as file:
