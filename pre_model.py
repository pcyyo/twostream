...
'''
data_loader = loader.spatio_temporal_dataloader(BATCH_SIZE=32, num_workers=8, in_channel=15,
...
feat_     = list(model.features.children())
class_    = list(model.classifier.children())
...
model.features   = nn.Sequential(*feat_)
model.classifier = nn.Sequential(*class_)
spat_model=model
temp_model=model

spat_model.load_state_dict(torch.load('pre_spat_params.pkl'))
temp_model.load_state_dict(torch.load('pre_temp_params.pkl'))
'''
if system has cuda 
'''
...
spat_scheduler = lr_scheduler.StepLR(spat_optimizer, step_size=5, gamma=0.1)
temp_scheduler = lr_scheduler.StepLR(temp_optimizer, step_size=5, gamma=0.1)

[spat_model, temp_model] = train_model(spat_model, spat_criterion, spat_optimizer, spat_scheduler,
                                       temp_model, temp_criterion, temp_optimizer, temp_scheduler, num_epochs = 10)